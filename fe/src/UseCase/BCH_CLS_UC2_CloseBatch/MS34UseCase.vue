...
export default {
  ...
  computed: {
    ...
    configurationObject() {
      return reactive(
        this.$store.getters["BCH_CLS_UC2_CloseBatch/gettersMegaSet34ConfigObj"]
      );
    },
  },
  methods: {
    onSubmit(values) {
     if (values.AccountNumberTextBox !== undefined) {
        ...
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setAccountNumber", values.AccountNumberTextBox);
      }
      if (values.AmountTextBox !== undefined) {
        ...
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setAmountTextBox", values.AmountTextBox);
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setSelectionConditionsRadioButton", values.SelectionConditionsRadioButton);
      }
      if (values.CurrencyDropDown !== undefined) {
        this.currencyDropDownHandler(values.CurrencyDropDown);
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setCurrencyDropDown", values.CurrencyDropDown);
      }
      if (values.DepartmentNumberDropDown !== undefined) {
        this.departmentNumberHandler(values.DepartmentNumberDropDown);
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setDepartmentNumberDropDown", values.DepartmentNumberDropDown);
      }
      if (values.TransactionStatusDropDown !== undefined) {
        this.transactionStatuHandler(values.TransactionStatusDropDown);
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setTransactionStatusDropDown", values.TransactionStatusDropDown);
      }
      if (values.SortingCriterionDropDown !== undefined) {
        this.sortingCriteriaOnChange(values.SortingCriterionDropDown);
        this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setSortingCriterionDropDown", values.SortingCriterionDropDown);
      }

      this.$store
          .dispatch("BCH_CLS_UC2_CloseBatch/MS34_OkButtonEvent", this.selectedCriteriaObj)
          .then((response) => {
            if (response) {
              this.$router.push({name: "BCH_CLS_UC2_CloseBatch_SuperSet42"});
            }
          }).catch((err) => {
            let errTitle = "Error!";
    ...
    selectionCriteriaOnChange(val) {
      this.$store.dispatch("BCH_CLS_UC2_CloseBatch/MS34_setSelectionCriteriaDropDown", val);

      // @author      [akash.kumar - 16-06-2022]
      // @desc        [assign the value to selectedSortingCriteriaTemp]
      this.selectedSortingCriteriaTemp = this.selectedCriteriaObj.selectedSortingCriteria;
      this.resetCriteriaObj();

      let showHideCompsObj = {
      ...
      this.$store.dispatch(
          "BCH_CLS_UC2_CloseBatch/MS34_showHideComponentsOnSelectionCriteria",
          showHideCompsObj
      ).then(() => {
    ...
    amountRadioBtnHandler(val) {
      if (val === "less_than")
        this.selectedCriteriaObj.amountOperand = "less";
    ...
    exitButtonClickHandler() {
      this.$store
          .dispatch("BCH_CLS_UC2_CloseBatch/MS34_ExitButtonEvent")
          .then((response) => {
            if (response)
