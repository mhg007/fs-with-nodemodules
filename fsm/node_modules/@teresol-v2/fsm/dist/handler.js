    ...
    if (key) {
        if (key in keys) {
            ...
            const newMachine = machine.withConfig({
                actions: {
                    sendPartialCtx: assign({
                        partialCtx: (context, event) => {
                            keys[key].sendingFunction(context.partialCtx, key);
                ...
                services: {
                    ...
                    sendFileWithContentType: async (context, event, { data }) => {
                        let apiURL = data.url;
                        let fetchOptions = {
                            method: "GET",
                            headers: { "Content-Type": "application/json" },
                        ...
                        return await fetch(apiURL, fetchOptions)
                            .then((response) => {
                                ...
                                if (response.status != 200) {
                                    ...
                                    return { content: response.body, contentType: response.headers.get('content-type') };
                                }
                            })
                            .then((body) => {
                                body.content.pipe(keys[key].res);
                                keys[key].res.type(body.contentType);
            ...
            if (jsonObj.header) {
                ...
                jsonObj.header.branchTypeName = req.headers["X-Decoded-BranchTypeName"];
                ...
                jsonObj.header.windowId = req.headers["X-Decoded-WindowId"];
            }
